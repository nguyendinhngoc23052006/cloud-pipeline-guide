...
| Secret key (`sb_secret_…`) | Supabase only | you never paste it anywhere — never code, Git, or Production-scoped Vercel (step 8.3) |
...
repo, redo step 6 and steps 8.1–8.2 — existing connections silently keep pointing
...
- A startup failure renders as visible text via the root layout or an `error.tsx` boundary — a deployment never shows a blank page.
- Components render UI; data access and validation live in `src/services/`.
- Read Supabase config from `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY ?? NEXT_PUBLIC_SUPABASE_ANON_KEY` — Next exposes `NEXT_PUBLIC_` to the browser natively, and these are the SAME names the Supabase→Vercel integration injects into previews, so production (set by hand in Vercel) and previews share one set of names with NO cross-prefix fallback. Never hardcode. If the stack changes, move the contract (this client, `.env.example`, and the Vercel production variable names) in ONE PR.
- Folders: `src/app` (routes) · `src/components` (UI) · `src/hooks` (logic) · `src/services` (data + validation) · `src/lib` (incl. `supabase/client.ts` + `supabase/server.ts`) · `src/middleware.ts` (session refresh) · `src/types` · `supabase/migrations` (one SQL file per change) · `supabase/config.toml` · `supabase/seed.sql`.
- **Designing structure on request** (`/prototype`, or "set up the project structure"): build it from my description — create only the feature/domain folders the project needs (a CRM → `contacts`, `deals`, `reminders`; a game → `game/{loop,scenes,entities}`), and omit the rest. Every folder gets a real, used starter file — never empty or `.gitkeep` shells. Wire it to the baseline: types in `src/types`, one core migration with RLS per table, reads through the Supabase clients in `src/lib/supabase/`, routes + placeholder components with loading/empty/error states. Record the layout in `docs/ARCHITECTURE.md`. Keep it a skeleton, not finished features. One PR into `main` with the "For you" block.
...
- [ ] key read from the `NEXT_PUBLIC_` Supabase names (browser) and the server client (server); nothing hardcoded; no secret in code
...
```text
Scaffold a MINIMAL Next.js (App Router) + TypeScript app that builds green and is ready to connect to Supabase and Vercel — nothing project-specific. Create:
- src/lib/supabase/client.ts (browser client) using createBrowserClient(NEXT_PUBLIC_SUPABASE_URL, NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY ?? NEXT_PUBLIC_SUPABASE_ANON_KEY); throw if URL or key is missing. (Both production and previews use NEXT_PUBLIC_ names — the integration injects the same names it always has, so no cross-prefix fallback is needed.)
- src/lib/supabase/server.ts (server client) using createServerClient from @supabase/ssr with the NEXT_PUBLIC_ URL and key from environment.
- src/middleware.ts for session refresh using the server client.
- A minimal src/app/ that compiles and renders one page (layout.tsx + page.tsx). No feature folders yet. If startup throws (e.g. missing Supabase config), render the error message as visible text via the root layout or an error.tsx boundary — a deployment must never show a blank page.
- No vercel.json — Next.js on Vercel handles routing natively.
- Mocked unit tests for the Supabase clients proving they build from the NEXT_PUBLIC_ names and show the readable error when they are absent.
...
- A committed package-lock.json; .github/dependabot.yml (weekly npm + github-actions, grouped; no runner field — Dependabot uses GitHub-hosted runners unless the "Dependabot on self-hosted runners" setting is ON); .env.example with the NEXT_PUBLIC_ vars; .gitignore ignoring .env.
...
## 7. Import to Vercel
1. vercel.com → **Add New → Project →** import your repo (Next.js is auto-detected,
   so there's nothing to set here); click **Deploy** without adding anything else
   (the build is green; the page errors at runtime until the variables exist —
   expected).
2. **Settings → Environment Variables →** add `NEXT_PUBLIC_SUPABASE_URL` and
   `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` (the two values you copied), selecting
   **Production** as the only environment for each; then open **Deployments** and
   on the newest one click **⋯ → Redeploy**. (These are the SAME names the
   integration injects into previews — no prefix change is needed in step 8.)
...
3. Back in Vercel's **Environment Variables**, delete any **Production**-scoped
   ...
   integration creates per PR: they're that preview's own keys, and they delete
   themselves when the PR closes.

*Note:* production values are scoped to **Production only** and carry the
`NEXT_PUBLIC_` names you typed (Production toggle ON, Preview and Development OFF
is the correct and intended state in Supabase's Vercel connection — Supabase
Branching injects per-PR preview credentials separately via the GitHub webhook;
turning Preview ON would incorrectly overwrite each preview's branch credentials
with the main project's credentials; Verified Jun 2026 — supabase.com discussion
#32596). Each PR's preview gets its **own** values
when a PR opens or a commit is pushed: the integration injects `NEXT_PUBLIC_SUPABASE_URL` and either
`NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` (new Supabase projects, Nov 2025+) or
`NEXT_PUBLIC_SUPABASE_ANON_KEY` (legacy projects) *(Verified Jun 2026 —
supabase.com discussions #29260, #40717)* — all under the `NEXT_PUBLIC_` prefix
Next.js exposes to the browser natively; the `?? NEXT_PUBLIC_SUPABASE_ANON_KEY`
fallback in the client handles legacy projects; new projects have
`NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` injected directly. Two health signs on any open PR: its **Supabase Preview** check is
green (not *skipped*), and about a minute after the first build the integration
...
Variables** shows `NEXT_PUBLIC_SUPABASE_URL` and either `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` or `NEXT_PUBLIC_SUPABASE_ANON_KEY` entries scoped to **Preview** with that
...
specifiers (e.g. npm:fflate, npm:unpdf) to the matching npm packages in
vitest.config.ts so Vitest runs the logic without a Deno runtime — keep the alias
versions in lockstep with the npm: strings. Open a PR into main.` No separate CI
job, no setup-deno, no sandbox exception — it rides the existing `tests` gate.
