...
```text
Scaffold a MINIMAL Next.js (App Router) + TypeScript app that builds green and is ready to connect to Supabase and Vercel — nothing project-specific. Use a `src/` directory. Create:
- src/lib/supabase/client.ts (browser) with createBrowserClient from @supabase/ssr, and src/lib/supabase/server.ts (server) with createServerClient reading cookies via cookies() from next/headers (getAll/setAll). Both read NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY ?? NEXT_PUBLIC_SUPABASE_ANON_KEY; throw a readable error if URL or key is missing. (Next exposes NEXT_PUBLIC_ to the browser natively; these are the SAME names the Supabase→Vercel integration injects into previews and the ones you set by hand in production — one set of names, no fallback.)
- src/middleware.ts refreshing the auth session on every request (the @supabase/ssr updateSession pattern).
- A minimal src/app/ that compiles and renders one page (layout.tsx + page.tsx). No feature folders yet. If startup throws (e.g. missing Supabase config), surface the message as visible text via the root layout or an error.tsx boundary — a deployment must never show a blank page. No vercel.json (Vercel builds Next natively; a SPA catch-all rewrite would break routing).
- Mocked unit tests for the Supabase clients proving they build from the NEXT_PUBLIC_ names and show the readable error when the key is absent.
...
- A committed package-lock.json; .github/dependabot.yml (weekly npm + github-actions, grouped; no runner field — Dependabot uses GitHub-hosted runners unless the "Dependabot on self-hosted runners" setting is ON); .env.example with the NEXT_PUBLIC_SUPABASE_* vars; .gitignore ignoring .env*.
...
2. **Settings → Environment Variables →** add `NEXT_PUBLIC_SUPABASE_URL` and
   ...
   **Production** as the only environment for each; then open **Deployments** and
   ...
   integration injects into previews — so the seam needs nothing configured.)
...
7. Back in Vercel's **Environment Variables**, delete any **Production**-scoped
   ...
   read them (only `NEXT_PUBLIC_` reaches the browser), and they delete themselves
   when the PR closes.
...
integration at PR-open under those SAME `NEXT_PUBLIC_` names (scoped to Preview), so
the one client reads the right values in each environment with no fallback and
nothing to configure on either dashboard. Two health signs on any open PR: its
**Supabase Preview** check is green (not *skipped*), and about a minute after the
first build the integration redeploys the preview by itself — that auto-redeploy is
the visible sign the sync ran.
...
Vitest config (`vitest.config.ts`) so Vitest runs the logic without a Deno runtime
— keep the alias versions in lockstep with the npm: strings. Open a PR into main.` No separate CI
