...
*Spans the Supabase client files (`src/lib/supabase/{client,server}.ts`),
`.env.example`, and the Vercel **Production** variable names · set in steps 5 and 7.* The textbook connected line: these name the same two values under one set of
`NEXT_PUBLIC_…` names (production and previews alike), so a change to any one moves
all of them or none.

```text
Change the env-name contract: <what you're changing>. Move ALL of it in ONE PR: the Supabase client files, .env.example, and the names I set in Vercel Production (all NEXT_PUBLIC_). In the "For you" block, give me the exact Vercel dashboard clicks for any Production-variable rename. Open ONE PR into main with the Self-check.
```
