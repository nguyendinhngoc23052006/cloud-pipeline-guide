...
- Read Supabase config from `VITE_SUPABASE_URL` and `VITE_SUPABASE_PUBLISHABLE_KEY ?? VITE_SUPABASE_ANON_KEY`; throw if URL or key is missing. Never hardcode. The contract spans `vite.config.ts` (`envPrefix: ['VITE_']`), this client, `.env.example`, and the Vercel production variable names — all using `VITE_` (step 8.3 configures the integration to inject `VITE_`-named vars into previews, matching production); if the stack changes, move all of it in ONE PR.
...
- One fact per tier, routed by scope: repo `MEMORY.md` = whole-scene facts ("the integration injects VITE_-named vars into previews after step 8.3") · folder `CLAUDE.md` = local wiring ("services/payment.ts re-derives amounts server-side") · agent memory = that agent's own lessons ("flagged a missing index in PR #12; pattern: filtered column"). If a fact fits two tiers, choose the narrowest.
...
- [ ] key read from `VITE_SUPABASE_URL` and `VITE_SUPABASE_PUBLISHABLE_KEY ?? VITE_SUPABASE_ANON_KEY`; `envPrefix: ['VITE_']`; nothing hardcoded; no secret in code
...
```text
Scaffold a MINIMAL Vite + React + TypeScript app that builds green and is ready to connect to Supabase and Vercel — nothing project-specific. Create:
- vite.config.ts with envPrefix: ['VITE_'] (step 8.3 sets the integration's per-connection prefix to VITE_, so previews receive the same names as production).
- src/lib/supabaseClient.ts using createClient(VITE_SUPABASE_URL, VITE_SUPABASE_PUBLISHABLE_KEY ?? VITE_SUPABASE_ANON_KEY); throw if URL or key is missing. (Both production and previews use VITE_ names — no cross-prefix fallback needed.)
- A minimal src/ that compiles and renders one page (main.tsx + App.tsx). No feature folders yet. main.tsx must import App dynamically; if startup throws (e.g. missing Supabase config), render the error message as visible text in #root — a deployment must never show a blank page.
- vercel.json with {"rewrites":[{"source":"/(.*)","destination":"/index.html"}]} — a Vite SPA on Vercel otherwise 404s on hard reloads of deep links.
- Mocked unit tests for supabaseClient proving it builds a client from the VITE_ names and shows the readable error when they are absent.
...
- A committed package-lock.json; .github/dependabot.yml (weekly npm + github-actions, grouped; no runner field — Dependabot uses GitHub-hosted runners unless the "Dependabot on self-hosted runners" setting is ON); .env.example with the VITE_ vars; .gitignore ignoring .env.
...
4. Back in Vercel's **Environment Variables**, delete any **Production**-scoped
   ...
   read them (Vite exposes only the `VITE_` prefix), and they delete themselves
   when the PR closes.
...
PR's preview gets its **own** credentials when a PR
opens or a commit is pushed: the integration injects `VITE_SUPABASE_URL` and either
`VITE_SUPABASE_PUBLISHABLE_KEY` (new Supabase projects, Nov 2025+) or
`VITE_SUPABASE_ANON_KEY` (legacy projects) *(Verified Jun 2026 — supabase.com
discussions #29260, #40717)* — the client reads the URL the same way in all
environments; production uses `VITE_SUPABASE_PUBLISHABLE_KEY` (what you set
manually in step 7.2); previews receive whichever key the integration provides —
the `?? VITE_SUPABASE_ANON_KEY` fallback in `supabaseClient.ts` handles legacy
projects; new projects have `VITE_SUPABASE_PUBLISHABLE_KEY` injected directly.
Two health signs on any open PR: its **Supabase Preview** check is green (not
*skipped*), and about a minute after the first build the integration redeploys
the preview by itself — that auto-redeploy is the visible sign the sync ran.

**✓** open one test PR; while it is open, **Vercel → Settings → Environment
Variables** shows `VITE_SUPABASE_URL` and either `VITE_SUPABASE_PUBLISHABLE_KEY` or `VITE_SUPABASE_ANON_KEY` entries scoped to **Preview** with that
...
vite.config.ts so Vitest runs the logic without a Deno runtime — keep the alias
