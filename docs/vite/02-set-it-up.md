...
3. In **Supabase → Project → Settings → Integrations → Vercel**, click **Manage**
   ...
   **Save** (previews now receive the same `VITE_` names you set in step 7.2 —
...
(step 8.3 set the prefix) — the client reads one set of names in all environments,
...
Variables** shows `VITE_SUPABASE_*` entries scoped to **Preview** with that
