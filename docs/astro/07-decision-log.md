...
- **Env-var seam:** Astro reaches Supabase on the server, so `src/middleware.ts` reads
  ...
  from `process.env`; step 8.3 sets the integration's per-connection prefix to `PUBLIC_`
...
- **Testing is layered like review, and every testing rule is verifiable.** Typed
  ...
  existing `tests` job** via `npm:`-specifier aliasing in `astro.config.mjs`'s `vite` key (kept in
...
- **Verified pass (Jun 2026, official docs):** Vercel's production branch moved to
  ...
  *post-merge* release gate that imports GitHub checks — they are not PR checks and
  ...
  Jun 2026; supabase/supabase PR #28058 merged Jul 2024); step 8.3 sets it to
...
- **Astro copy — framework deltas (Verified Jun 2026, official docs).** This is the
  ...
  `PUBLIC_SUPABASE_PUBLISHABLE_KEY ?? PUBLIC_SUPABASE_ANON_KEY` from `process.env`; step 8.3
