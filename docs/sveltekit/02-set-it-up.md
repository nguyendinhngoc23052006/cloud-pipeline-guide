...
- Read Supabase config on the SERVER: resolve `PUBLIC_SUPABASE_URL ?? NEXT_PUBLIC_SUPABASE_URL` and `PUBLIC_SUPABASE_PUBLISHABLE_KEY ?? NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY ?? NEXT_PUBLIC_SUPABASE_ANON_KEY` in `hooks.server.ts`, then pass the values to the browser via the root `+layout` load. SvelteKit's public `$env` exposes only one prefix, so the server (which sees every variable regardless of prefix) bridges production's `PUBLIC_` names and previews' injected `NEXT_PUBLIC_` names. Never hardcode. The contract spans `hooks.server.ts`, `src/routes/+layout.server.ts` / `+layout.ts`, `.env.example`, and the Vercel production variable names; if the stack changes, move all of it in ONE PR, and never treat `PUBLIC_` as permanent.
...
- [ ] key resolved server-side (`PUBLIC_ ?? NEXT_PUBLIC_`) in hooks and passed to the client via the layout load; nothing hardcoded; no secret in code
...
- src/hooks.server.ts creating a @supabase/ssr server client with createServerClient(url, key, { cookies from event.cookies getAll/setAll }); resolve url/key server-side as PUBLIC_SUPABASE_URL ?? NEXT_PUBLIC_SUPABASE_URL and PUBLIC_SUPABASE_PUBLISHABLE_KEY ?? NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY ?? NEXT_PUBLIC_SUPABASE_ANON_KEY (PUBLIC_ via $env/dynamic/public, the injected NEXT_PUBLIC_ via $env/dynamic/private — SvelteKit's public $env takes one prefix, so the server bridges them); throw if url or key is missing. Put supabase + a safeGetSession helper on event.locals.
...
PR-open, injected under the integration's fixed `NEXT_PUBLIC_` names — `hooks.server.ts`
resolves `PUBLIC_ ?? NEXT_PUBLIC_` server-side and passes the values to the browser via
the layout load (step 4), so nothing needs configuring on either dashboard. Two health signs on any open PR: its **Supabase Preview** check
...
1. The server-side PUBLIC_ ?? NEXT_PUBLIC_ resolver in hooks.server.ts (passed to the client via the layout load) — retirable when the Supabase→Vercel integration lets you choose the injected variable names (so it can inject PUBLIC_ names directly).
